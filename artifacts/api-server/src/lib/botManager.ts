...
class BotManager {
  ...
  private async positionTick() {
    if (!this.running || this.positionCheckInFlight || this.tickInFlight) return;
  ...
  private async tick() {
    ...
    try {
      ...
      this.tickCount += 1;
      try { await this.syncWithExchange("auto"); } catch (err) { logger.warn({ err }, "Auto-sync failed"); }
  ...
  async syncWithExchange(mode: "auto" | "adopt" = "adopt"): Promise<{ added: number; removed: number; details: string[] }> {
    ...
    const dbKeys = new Set(dbPositions.filter((p) => p.triggeredBy !== "paper").map((p) => `${p.symbol}|${p.side}`));
    for (const ex of exchangePositions) {
      ...
      if (mode === "auto") {
        ...
        await this.addLog("info", `거래소에 봇이 추적하지 않는 포지션 발견: ${sym} ${ex.side} qty=${ex.quantity} (수동 동기화 버튼으로 인계 가능)`, sym);
        continue;
      }
      ...
      try {
        await db.insert(activePositionsTable).values({ symbol: sym, side: ex.side, entryPrice: ex.entryPrice, quantity: ex.quantity, takeProfit: tp, stopLoss: sl, triggeredBy: "sync" });
        added++;
    ...
    if (removed > 0) await this.addLog("warning", `거래소↔DB 동기화: DB 정리 -${removed} (거래소에 없는 phantom 포지션 제거)`).catch(() => {});
    if (added > 0) await this.addLog("info", `거래소↔DB 동기화: 거래소→DB +${added}`).catch(() => {});
  ...
  private mtfAligned(primaryBias: string, mtf: Record<string, string>): { aligned: boolean; conflicts: string[] } {
    ...
    const conflicts = Object.entries(mtf).filter(([, b]) => b !== "neutral" && b !== primaryBias).map(([tf, b]) => `${tf}=${b}`);
  ...
  private async processSymbol(symbol: string, config: BotConfigRow) {
    ...
    if (candles.length < 50) { await this.addLog("warning", `${symbol} 캔들 데이터 부족`, symbol); return; }
    ...
    if (!hasDivergence) { await this.addLog("info", `${symbol} @ $${ticker.price.toFixed(2)} — 다이버전스 없음 (관망, AI 호출 스킵)`, symbol); return; }
    const strongEnough = divergence.bullishCount >= 2 || divergence.bearishCount >= 2;
    if (!strongEnough) { await this.addLog("info", `${symbol} @ $${ticker.price.toFixed(2)} — 신호 약함 (강세 ${divergence.bullishCount} / 약세 ${divergence.bearishCount}, AI 호출 스킵)`, symbol); return; }
    ...
    await db.insert(aiSignalsTable).values({
      symbol, timeframe: config.timeframe, action: decision.action, confidence: decision.confidence,
      riskLevel: decision.riskLevel, currentPrice: ticker.price, entryPrice: decision.suggestedEntryPrice,
      takeProfit: decision.suggestedTakeProfit, stopLoss: decision.suggestedStopLoss,
      ...
      reasoning: decision.reasoning, bullishCount: divergence.bullishCount, bearishCount: divergence.bearishCount,
    ...
    await this.addLog("trade", `신호 ${decision.action} @ $${decision.suggestedEntryPrice.toFixed(2)} — TP $${tp.toFixed(2)} (${tpPct >= 0 ? "+" : ""}${tpPct.toFixed(2)}%) / SL $${sl.toFixed(2)} (${slPct >= 0 ? "+" : ""}${slPct.toFixed(2)}%) — ${moveTxt} | 신뢰도 ${(decision.confidence * 100).toFixed(0)}% — ${decision.reasoning}`, symbol, decision.action);
    let params = resolveEffectiveParams(config, symbol);
    if (params.overridden.length > 0) await this.addLog("info", `${symbol} 심볼별 오버라이드 적용: ${params.overridden.join(", ")} (금액 $${params.tradeAmount}, 신뢰도 ${params.minConfidence}, TP ${params.takeProfitPercent}% / SL ${params.stopLossPercent}%)`, symbol);
    if (!config.autoTrade) { await this.addLog("info", `${symbol} 진입 스킵 — 자동매매 OFF (신호: ${decision.action}, 신뢰도 ${(decision.confidence * 100).toFixed(0)}%)`, symbol); return; }
...

    {
      ...
      if (!isPaper && (config.entryMode ?? "fixed") === "full") {
        try {
          ...
          if (allIn <= 0) { await this.addLog("warning", `${symbol} 진입 스킵 — 가용 USDT 잔고 없음 ($${free.toFixed(2)})`, symbol); return; }
          await this.addLog("info", `${symbol} 풀 진입 모드 — 가용 잔고 $${free.toFixed(2)} × 99% = 증거금 $${allIn} 사용`, symbol);
          params = { ...params, tradeAmount: allIn };
        } catch (err) {
          await this.addLog("error", `${symbol} 진입 스킵 — 잔고 조회 실패: ${err instanceof Error ? err.message : String(err)}`, symbol);
          return;
        }
      }
      ...
      try {
        ...
        try {
          ...
          try {
            const recent = await db.select().from(aiSignalsTable).where(eq(aiSignalsTable.symbol, symbol)).orderBy(desc(aiSignalsTable.createdAt)).limit(1);
            if (recent.length > 0) { bullishCount = recent[0].bullishCount; bearishCount = recent[0].bearishCount; }
          } catch { /* best-effort */ }
          const [reflectionRow] = await db.insert(tradeReflectionsTable).values({
            symbol, timeframe: config.timeframe, side, entryPrice, exitPrice: entryPrice, exitReason: "ENTRY_FAILED",
            pnl: 0, pnlPercent: 0, holdSeconds: 0, originalConfidence: decision.confidence,
            originalExpectedMovePercent: decision.expectedMovePercent, originalReasoning: decision.reasoning,
            bullishCount, bearishCount, lessonText: `진입 실패 사유: ${msg}`,
          }).returning();
          if (reflectionRow) {
            this.writeReflectionLesson(reflectionRow.id, { symbol, timeframe: config.timeframe, side, entryPrice, exitPrice: entryPrice, exitReason: "ENTRY_FAILED", pnlPercent: 0, holdSeconds: 0, originalConfidence: decision.confidence, originalExpectedMovePercent: decision.expectedMovePercent, originalReasoning: decision.reasoning, bullishCount, bearishCount, leverage: config.leverage ?? 10, failureMessage: msg }).catch((e) => logger.warn({ err: e, id: reflectionRow.id }, "Failed-entry reflection lesson generation failed"));
          }
        } catch (refErr) {
  ...
  private sanitizeDecision(decision: AiDecision, currentPrice: number, atrPercent: number | null): AiDecision {
    if (decision.action === "HOLD") return { ...decision, expectedMovePercent: 0, suggestedTakeProfit: null, suggestedStopLoss: null };
    const dir = decision.action === "BUY" ? 1 : -1;
    ...
    if (!tpDistanceOk) tp = entry * (1 + dir * expectedMoveAbs);
    if (!slDistanceOk) { const slMoveAbs = Math.max(0.001, expectedMoveAbs * 0.5); sl = entry * (1 - dir * slMoveAbs); }
    const minStopPct = Math.max(0.0015, (atrPercent ?? 0.3) / 100 * 0.5);
    const slDistance = Math.abs((sl as number) - entry) / entry;
    if (slDistance < minStopPct) sl = entry * (1 - dir * minStopPct);
    const tpDistance = Math.abs((tp as number) - entry) / entry;
    const slDistance2 = Math.abs((sl as number) - entry) / entry;
    if (tpDistance < slDistance2 * 1.2) tp = entry * (1 + dir * Math.max(slDistance2 * 1.2, expectedMoveAbs));
    return { ...decision, expectedMovePercent: movePct, suggestedEntryPrice: entry, suggestedTakeProfit: tp, suggestedStopLoss: sl };
  }
  ...
  private async manageActivePositions(config?: BotConfigRow) {
    ...
    await Promise.allSettled(positions.map(async (pos) => {
      let price: number;
      try { price = (await exchangeService.getTicker(pos.symbol)).price; }
      catch (err) { logger.warn({ err, symbol: pos.symbol }, "Failed to fetch ticker for position"); return; }
      ...
      if (cfg.useTrailingStop) {
        ...
        if (moveFromEntryPct >= activate) {
          ...
          const newSl = isLong ? newHwm * (1 - distance / 100) : newHwm * (1 + distance / 100);
          const slImproved = isLong ? newSl > pos.stopLoss : newSl < pos.stopLoss;
          if (newHwm !== hwm || slImproved) {
            await db.update(activePositionsTable).set({ highWaterMark: newHwm, ...(slImproved ? { stopLoss: newSl } : {}) }).where(eq(activePositionsTable.id, pos.id));
            if (slImproved) { pos.stopLoss = newSl; await this.addLog("info", `${pos.symbol} 트레일링 SL 업데이트 → $${newSl.toFixed(4)} (HWM $${newHwm.toFixed(4)})`, pos.symbol); }
          }
        }
      }
      if (cfg.usePartialTp && !pos.partialTpDone) {
        ...
        if (tpHit) {
          try {
            ...
            if (!isPaperPos) { try { const onEx = await exchangeService.getPositionAmount(pos.symbol, positionSide); if (onEx > 0) actualQty = onEx; } catch { /* fall back */ } }
            const partPct = (cfg.partialTpPercent ?? 50) / 100;
            const closeQty = actualQty * partPct;
            if (closeQty > 0) {
              if (!isPaperPos) await exchangeService.placeOrder(pos.symbol, isLong ? "sell" : "buy", "market", closeQty, undefined, { positionSide, reduceOnly: true });
              const realized = (price - pos.entryPrice) * closeQty * (isLong ? 1 : -1);
              await db.insert(tradeHistoryTable).values({ symbol: pos.symbol, side: isLong ? "sell" : "buy", price, quantity: closeQty, total: price * closeQty, fee: price * closeQty * 0.001, pnl: realized, triggeredBy: isPaperPos ? "paper" : "bot" });
              const dir = isLong ? 1 : -1;
              const extendedTp = pos.entryPrice + dir * Math.abs(pos.takeProfit - pos.entryPrice) * 2;
              await db.update(activePositionsTable).set({ quantity: actualQty - closeQty, stopLoss: pos.entryPrice, takeProfit: extendedTp, partialTpDone: true }).where(eq(activePositionsTable.id, pos.id));
              pos.stopLoss = pos.entryPrice; pos.takeProfit = extendedTp; pos.quantity = actualQty - closeQty;
      ...
      try {
        ...
        if (!isPaperPos) {
          try {
            ...
            if (onExchange <= 0) { await db.delete(activePositionsTable).where(eq(activePositionsTable.id, pos.id)); await this.addLog("warning", `${pos.symbol} ${positionSide} 거래소에 실제 포지션이 없어 DB만 정리`, pos.symbol); return; }
            actualQty = onExchange;
          } catch (err) { logger.warn({ err, symbol: pos.symbol }, "Failed to fetch on-exchange position; falling back to DB qty"); }
        ...
        try {
          const recent = await db.select().from(aiSignalsTable).where(eq(aiSignalsTable.symbol, pos.symbol)).orderBy(desc(aiSignalsTable.createdAt)).limit(1);
          if (recent.length > 0) { bullishCount = recent[0].bullishCount; bearishCount = recent[0].bearishCount; timeframe = recent[0].timeframe; if (originalConfidence == null) originalConfidence = recent[0].confidence; if (!originalReasoning) originalReasoning = recent[0].reasoning; if (originalExpectedMovePercent == null) originalExpectedMovePercent = recent[0].expectedMovePercent; }
        } catch { /* best-effort */ }
        const [reflectionRow] = await db.insert(tradeReflectionsTable).values({ symbol: pos.symbol, timeframe, side: pos.side, entryPrice: pos.entryPrice, exitPrice: price, exitReason, pnl, pnlPercent: pnlPct, holdSeconds, originalConfidence, originalExpectedMovePercent, originalReasoning, bullishCount, bearishCount }).returning();
        await db.delete(activePositionsTable).where(eq(activePositionsTable.id, pos.id));
        await this.addLog(exitReason === "TP" ? "trade" : "warning", `${isPaperPos ? "[가상] " : ""}${exitReason} 청산: ${pos.symbol} @ $${price.toFixed(2)} | 진입 $${pos.entryPrice.toFixed(2)} | P&L ${pnl >= 0 ? "+" : ""}$${pnl.toFixed(2)} (${pnlPct >= 0 ? "+" : ""}${pnlPct.toFixed(2)}%)`, pos.symbol, isLong ? "SELL" : "BUY");
        notifyExit({ symbol: pos.symbol, side: pos.side, exitReason, entryPrice: pos.entryPrice, exitPrice: price, pnl, pnlPercent: pnlPct, holdMinutes: Math.floor(holdSeconds / 60), isPaper: isPaperPos }).catch((err) => logger.warn({ err }, "Failed to send exit notification"));
        if (reflectionRow) {
          this.writeReflectionLesson(reflectionRow.id, { symbol: pos.symbol, timeframe: timeframe ?? "15m", side: pos.side, entryPrice: pos.entryPrice, exitPrice: price, exitReason, pnlPercent: pnlPct, holdSeconds, originalConfidence: originalConfidence ?? null, originalExpectedMovePercent: originalExpectedMovePercent ?? null, originalReasoning: originalReasoning ?? null, bullishCount, bearishCount, leverage }).catch((err) => logger.warn({ err, id: reflectionRow.id }, "Reflection lesson generation failed"));
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
  ...
  private async writeReflectionLesson(reflectionId: number, ctx: {
    symbol: string; timeframe: string; side: string; entryPrice: number; exitPrice: number;
    exitReason: string; pnlPercent: number; holdSeconds: number;
    originalConfidence: number | null; originalExpectedMovePercent: number | null;
    originalReasoning: string | null; bullishCount: number; bearishCount: number;
    leverage?: number; failureMessage?: string;
  }) {
    ...
    const results = await Promise.allSettled(roles.map(async (role) => {
      const msg = await anthropic.messages.create({ model: ANTHROPIC_MODEL, max_tokens: 512, system: BotManager.ROLE_SYSTEMS[role], messages: [{ role: "user", content: `다음 거래 결과에 대해 반성 노트를 작성하세요.\n\n${tradeInfo}` }] });
      const block = msg.content[0];
      return block && block.type === "text" ? `[${role}]\n${block.text.trim()}` : "";
    }));
    const lessonParts = results.filter((r): r is PromiseFulfilledResult<string> => r.status === "fulfilled" && !!r.value).map((r) => r.value);
    if (lessonParts.length === 0) return;
    await db.update(tradeReflectionsTable).set({ lessonText: lessonParts.join("\n\n") }).where(eq(tradeReflectionsTable.id, reflectionId)).catch((err) => logger.warn({ err, reflectionId }, "Failed to save reflection lesson"));
  }

  private async fetchRecentReflections(symbol: string): Promise<string> {
    try {
      const since = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      const rows = await db.select().from(tradeReflectionsTable).where(and(eq(tradeReflectionsTable.symbol, symbol), gte(tradeReflectionsTable.createdAt, since))).orderBy(desc(tradeReflectionsTable.createdAt)).limit(6);
  ...
  private async fetchSignalStats(symbol: string): Promise<string> {
    try {
      ...
      const rows = await db.select({ side: tradeReflectionsTable.side, bullishCount: tradeReflectionsTable.bullishCount, bearishCount: tradeReflectionsTable.bearishCount, exitReason: tradeReflectionsTable.exitReason, pnlPercent: tradeReflectionsTable.pnlPercent }).from(tradeReflectionsTable).where(and(eq(tradeReflectionsTable.symbol, symbol), gte(tradeReflectionsTable.createdAt, since)));
      ...
      for (const r of rows) {
        ...
        cell.total++; if (r.exitReason === "TP") cell.wins++; cell.pnlSum += r.pnlPercent;
        map.set(key, cell);
      }
      if (map.size === 0) return "  (유효한 통계 없음)";
      const lines = [...map.entries()].sort((a, b) => b[1].total - a[1].total).slice(0, 10).map(([key, { wins, total, pnlSum }]) => {
        const [side, bPart, sPart] = key.split("|");
        const bull = bPart.replace("b", ""); const bear = sPart.replace("s", "");
        const winRate = ((wins / total) * 100).toFixed(0); const avgPnl = (pnlSum / total).toFixed(2);
        return `  - ${side.toUpperCase()} 강세${bull === "4" ? "4+" : bull}/약세${bear === "4" ? "4+" : bear}: 승률 ${winRate}% (${wins}/${total}건) 평균손익 ${Number(avgPnl) >= 0 ? "+" : ""}${avgPnl}%`;
      });
      return lines.join("\n");
    } catch (err) {
...
Respond ONLY with valid JSON (no markdown, no prose):
{
  ...
  "suggestedStopLoss": number
    ...
    const frText = fundingRate !== null ? `Funding rate: ${(fundingRate * 100).toFixed(4)}%` : "Funding rate: unavailable";
    const oiText = openInterest !== null ? `Open interest: ${openInterest.toLocaleString()} contracts` : "Open interest: unavailable";
    const [reflections, signalStats] = await Promise.all([this.fetchRecentReflections(symbol), this.fetchSignalStats(symbol)]);
...
중요: 복기 데이터가 20건 미만이면 복기 노트와 통계는 완전히 무시하고 현재 기술적 신호(다이버전스 강도, ATR, 펀딩비)만으로 판단하세요. 데이터 부족 상태에서 과거 손절 패턴을 일반화하지 마세요. confidence는 신호 강도와 ATR만 기준으로 산출하세요.`;
    const message = await anthropic.messages.create({ model: ANTHROPIC_MODEL, max_tokens: 1024, system: systemPrompt, messages: [{ role: "user", content: userMessage }] });
    const block = message.content[0];
    const content = block && block.type === "text" ? block.text : "";
    if (!content) throw new Error("Empty AI response");
    let parsed: ReturnType<typeof JSON.parse>;
    try { parsed = JSON.parse(extractJson(content)); }
    catch { logger.warn({ content }, "Failed to parse AI JSON response"); throw new Error("AI response JSON parse failed"); }
    ...
    if (action === "HOLD") { takeProfit = null; stopLoss = null; }
